...
namespace Proyecto_Final_Blood_Bank
{
	...
	0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
		...
		private void Register_Click(object sender, EventArgs e)
		{
			...
			{
				// Hashear la contraseña usando SHA256 y luego codificar en Base64
				string hasheado = SHA256(Base64Encode(temp_pwd));

				// Consulta para verificar si el usuario ya existe
				string login = "SELECT * FROM Personal WHERE Usuario = @usuario";

				// Consulta parametrizada para insertar un nuevo usuario
				string comando = "INSERT INTO Personal VALUES (@usuario, @password, @estado)";
				...
				using (SqlConnection con = new SqlConnection("Data Source=localhost;Initial Catalog=Hospital;Integrated Security=True"))
				{
					...
					{
						cmd1.Parameters.AddWithValue("@usuario", temp_usuario);
					...
					using (SqlCommand cmd = new SqlCommand(comando, con))
					{
						...
						cmd.Parameters.AddWithValue("@password", hasheado);
						cmd.Parameters.AddWithValue("@estado", "N");

						cmd.ExecuteNonQuery();

						MessageBox.Show("La cuenta fue creada exitosamente", "Cuenta creada", MessageBoxButtons.OK, MessageBoxIcon.Information);
						txtConfirm.Clear();
						txtUsername.Clear();
						txtPassword.Clear();
					}
